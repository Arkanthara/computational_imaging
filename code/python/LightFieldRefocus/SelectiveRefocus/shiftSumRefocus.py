...
def shiftSumRefocus(lightField, arrayLength, arrayDepth, shiftMat, depth):
    """
    ...
    refocusedImage = np.zeros((1088,2048,3))
    index = 0
    for i in range(arrayLength):
        for j in range(arrayDepth):
            index = index + 1
            # Perform Shift and Sum
            refocusedImage = refocusedImage + np.double(np.array(imtranslate(lightField[:,:,:,index], [-1*d*shiftMat[i,j,0], -1*d*shiftMat[i,j,1]])))

...
def selectiveBlurring(img: np.ndarray, imgDepth: np.ndarray, numLevels: int, focusRange: list, upperFilt: int):
    ...
    uLimit = mode(imgDepth).mode[0] + 30
    ...
    bins = np.linspace(0, float(uLimit), float(numLevels))
    ...
    chooseFilt = np.zeros((numLevels)) # Initialize with all 0
    ...
    for j in range(numBinsBelow+2, numLevels-numBinsAbove):
        chooseFilt[j] = 1
    ...
    if((numBinsAbove!=0) or (numBinsBelow!=0)):
        ...
        if(numBinsBelow!=0):
            for j in range(numBinsBelow, 0, -1):
                chooseFilt[j-1] = chooseFilt[j]+1
    ...
    filt = np.uint8(np.zeros((sizeImg[0],sizeImg[1],sizeImg[2],numLevels)))
